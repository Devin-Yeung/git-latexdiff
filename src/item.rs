...
impl SkimItem for Item {
    ...
    fn preview(&self, _context: PreviewContext) -> ItemPreview {
        let commit = self.repo.find_commit(self.oid).unwrap();
        let oid = format!("{}", commit.id());
        ItemPreview::AnsiText(format!("commit {}\nAuthor: {}\n\n{}", oid.yellow(), commit.author().to_string(), commit.message().unwrap()))
    }
    ...
    fn output(&self) -> Cow<str> {
        ...
        let summary = commit.summary().unwrap();
        let oid = format!("{}", commit.id());
        return Cow::from(String::from(format!("{} {} has been selected.", &oid[0..7], summary)));
    }
}